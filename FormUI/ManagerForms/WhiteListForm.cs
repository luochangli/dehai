...
namespace FormUI.ManagerForms
{
    public partial class WhiteListForm : Form
    {
        ...
        private void GetWhiteList()
        {
            ...
            List<WhiteList> whiteLists = _service.GetModelList();
        ...
        private void btImportWhiteList_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                foreach (WhiteList whiteList in WhiteListSerializer(str))
                {
        ...
        {
            //文件里面应该包含"."结束符
            int lastIndex = str.IndexOf(".", StringComparison.Ordinal);
            if (lastIndex < 1) throw new FileLoadException("请检查白名单文件，应以\".\"结尾");

            //第一个"."结束符之前的内容，才是真正的内容
            string realContent = str.Substring(0, lastIndex);

            //中控机号码&管理号码与普通终端号码的分隔符";"位置
            int splitIndex = realContent.LastIndexOf(";", StringComparison.Ordinal);
            ...
            {
                //取出中控机号码+管理号码
                managerArray = realContent.Substring(0, splitIndex)
            ...
            //然后下面的都是管理号码
            for (int i = index; i < managerArray.Length; i++)
            {
                yield return new WhiteList
                    {
            ...
            //后面的是普通终端号码
            string[] userArray = realContent.Substring(splitIndex, realContent.Length - splitIndex)
                                            .Split(new[] {",", "."}, StringSplitOptions.RemoveEmptyEntries);
        ...
        private string WhiteListDeserializer(IEnumerable<WhiteList> whiteLists)
        {
            ...
            return (centre + manager).TrimEnd(new[] {Convert.ToChar(",")}) + ";" +
                   user.TrimEnd(new[] {Convert.ToChar(",")}) + ".";
        }
