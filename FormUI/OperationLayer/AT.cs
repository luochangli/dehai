...
{
    internal delegate void AsyncSend(string terminalName, string phone, string context);
    ...
    public class AT
    {
        ...
        ///     短信结束符
        /// </summary>
        private readonly byte[] END_OF_SMS = SerialPortUtil.HexToByte("1A");
        ...
        ///     6D4B8BD5：短信内容：一
        /// </summary>
        public static void GetSmsContent(string hex, out bool isLongMessage, out string phone, out DateTime time,
                                         out string content, out int current, out int total, out string identifier)
        {
            //前部分长度
            int head = 0;
            //短信中心号码长度
            int hex_smsc_length = 2;
            //PDU Type长度
            int hex_pdu_type_length = 2;
            //目标地址长度
            int hex_phone_length = 2;
            //目标地址格式长度
            int hex_phone_format_length = 2;
            //协议标识(TP-PID)长度
            int hex_pid = 2;
            //用户信息编码方式(TP-DCS)长度
            int hex_dcs = 2;
            //发送时间长度
            int hex_time_length = 14;
            //长短信标识头长度
            int hex_long_sms_protocol_head_length = 2;
            //短信内容长度
            int hex_content_length = 2;
            ...
            int smsc_phone_length = Convert.ToInt32(hex.Substring(0, hex_smsc_length), 16)*2;
            ...
            int phone_length = Convert.ToInt32(hex.Substring(head, hex_phone_length), 16);
            head += hex_phone_length;
            head += hex_phone_format_length;
            if (phone_length%2 != 0)
                phone_length += 1;
            string phoneHex = hex.Substring(head, phone_length);
            phone = PhoneDecoder(phoneHex);
            head += phone_length;
            head += hex_pid;
            int dcs = Convert.ToInt32(hex.Substring(head, hex_dcs), 16);
            head += hex_dcs;
            time = HexToTime(hex.Substring(head, hex_time_length));
            head += hex_time_length;
            int content_length = Convert.ToInt32(hex.Substring(head, hex_content_length), 16);
            head += hex_content_length;
            if (isLongMessage)
            {
                int procotol_length = HexToDec(hex.Substring(head, hex_long_sms_protocol_head_length))*2;
            ...
            string hex_content = hex.Substring(head, hex.Length - head);
            switch (dcs)
            {
                case ENGLISH_CODE:
...
//                    content = PDU7bitDecoder(hex_content, content_length);
        ...
        {
            int str1 = int.Parse(pduType, NumberStyles.HexNumber);
            int str2 = str1 & 0x40;
        ...
        {
            char[] charArray = HexReverse(hex).ToCharArray();
            Array.Reverse(charArray);
            string binary = HexToBinary(new string(charArray));
