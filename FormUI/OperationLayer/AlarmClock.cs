...
namespace FormUI.OperationLayer
{
    public class AlarmClock
    {
        ...
        private void Regular()
        {
            ...
            foreach (RegularPlay item in entities)
            {
                ...
                if (type[0] == "每月")
                {
                    ...
                    {
                        foreach (RegularPlay term in entities)
                        {
                            if (term.PlayType.Equals(item.PlayType) && term.Time.Equals(item.Time))
                            {
                                SendOrder(term);
                            }
                        }
                ...
                if (type[0] == "指定时间")
                {
                    string time = type[1] + "-" + type[2] + "-" + type[3] + " " + item.Time;
        ...
        {
            _order.PlayMusic(item.TerminalName, item.Phone, item.PlayType, item.Music,
                             item.PlayTimes);
        ...
        private void SendOrder1(RegularPlay item)
        {
