...
namespace FormUI.SettingForms
{
    ...
    {
        private readonly TerminalService _bllTerminal;
        ...
        private void GetTerminalList()
        {
            ...
            List<Terminal> lists = _bllTerminal.GetModelList("");
            _terminals = lists;
            bs.DataSource = lists;
        ...
        {
            string[] strcontent = str.Split(new[] {",", "."}, StringSplitOptions.RemoveEmptyEntries);
            if (strcontent.Length != 2) throw new FileLoadException("终端表导入的格式不对！所有标点符号都是英文符号,.;|。");
            string content = strcontent[1];
            string[] strmodel = content.Split(new[] {";"}, StringSplitOptions.RemoveEmptyEntries);
            for (int i = 0; i < strmodel.Length; i++)
            {
                string[] model = strmodel[i].Split(new[] {"|"}, StringSplitOptions.RemoveEmptyEntries);
                if (model.Length == 4)
                {
