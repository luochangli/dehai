...
namespace TomorrowSoft.DAL
{
    ...
    public class ConditionRepository
    {
        ...
        public bool Add(Condition model)
        {
            ...
            strSql.Append(
                "@Photovoltaic,@Battery,@PowerSource,@QS,@Horn,@Charge,@Discharge,@Volume,@HandlerTime,@PhoneNo,@Name)");
            SQLiteParameter[] parameters =
                {
        ...
        ///     更新一条数据
        /// </summary>
        public bool Update(Condition model)
        {
            var strSql = new StringBuilder();
...
//			strSql.Append(" where Id in ("+Idlist + ")  ");
        ...
        ///     得到一个对象实体
        /// </summary>
        public Condition GetModel(int Id)
        {
            var strSql = new StringBuilder();
            strSql.Append(
                "select Id,Photovoltaic,Battery,PowerSource,RDS,QS,Horn,Charge,Discharge,Volume,HandlerTime,PhoneNo from Condition ");
            strSql.Append(" where Id=@Id ");
            SQLiteParameter[] parameters =
                {
                    new SQLiteParameter("@Id", DbType.Int32)
            ...
            if (ds.Tables[0].Rows.Count > 0)
            {
                ...
                if (ds.Tables[0].Rows[0]["Photovoltaic"] != null &&
                    ds.Tables[0].Rows[0]["Photovoltaic"].ToString() != "")
                {
        ...
        public DataSet GetList(string strWhere)
        {
            var strSql = new StringBuilder();
            strSql.Append(
                "select Id,Photovoltaic,Battery,PowerSource,QS,Horn,Charge,DisCharge,Volume,HandlerTime,PhoneNo,Name ");
