...
namespace TomorrowSoft.DAL
{
    public class LongSmsRepository
    {
        ...
        public IList<LongSms> GetBy(string phone, string identifier, DateTime time)
        {
            ...
            strSql.Append(" where Phone=@Phone and Identifier=@Identifier and datetime([Time]) >= datetime('" +
                          time.AddMinutes(-15).ToString("yyyy-MM-dd HH:mm:ss") + "') and datetime([Time]) <= datetime('" +
                          time.AddMinutes(15).ToString("yyyy-MM-dd HH:mm:ss") + "') ORDER BY Current ASC ");
                //, dtpEnd.Value.ToString("yyyy-MM-dd HH:mm")");
            SQLiteParameter[] parameters =
                {
                    new SQLiteParameter("@Phone", DbType.String),
                    new SQLiteParameter("@Identifier", DbType.String)
            ...
            DataTable dt = DbHelperSQLite.Query(strSql.ToString(), parameters).Tables[0];
